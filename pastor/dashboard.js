...
async function handleApply() {
  ...
  // Registration requirements were tightened — an ordination certificate is
  // now mandatory, not optional.
  const certFileCheck = document.getElementById('cert-file')?.files?.[0];
  if (!certFileCheck) return showAlert('apply-error', 'An ordination certificate is required to apply.');
...
async function clearAllInbox() {
  ...
  if (isNotifActive) {
    if (!confirm('Clear all notifications and sent messages? This cannot be undone.')) return;
    try {
      ...
      loadInbox();
      updateBadges();
      showToast('All notifications and messages cleared');
    } catch(e) { showToast('Failed to clear notifications', 'error'); }
  } else {
...
async function viewSermon(id) {
  ...
  document.body.appendChild(overlay);
  } catch(e) { console.error('viewSermon error:', e); alert('Could not load sermon.'); }
}

...
async function pdOpenBillingPortal(){
  if (!confirm(pdTr('cancel_subscription_confirm'))) return;
  try {
    const data = await api('/api/billing/cancel-subscription', 'POST');
    if (data?.error) { showToast(data.error, 'error'); return; }
    showToast(pdTr('cancellation_requested'));
    pdLoadProSection();
  } catch (e) {
    showToast(e.message || pdTr('could_not_cancel_subscription'), 'error');
  }
}
...
async function checkAndShowProBanner() {
  ...
  try {
    ...
    banner.innerHTML = `
      ...
      <button onclick="pdStartCheckout()" style="background:#D4AF37;border:none;color:#071528;font-weight:700;padding:10px 20px;border-radius:10px;font-size:14px;cursor:pointer;width:100%;margin-top:16px;">${pdTr('upgrade_to_pro_dollar')}</button>
    `;
